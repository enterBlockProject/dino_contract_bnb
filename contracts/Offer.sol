...
contract Offer {
    ...
    function newOffering(
        ...
        IBEP20(dino20).safeTransferFrom(msg.sender, address(this), initialSupply);
    }
    ...
    function deposit(address dino20, uint amount) public {
        ...
        IBEP20(address(dino)).safeTransferFrom(msg.sender, address(this), amount);
    ...
    function withdraw(address dino20, uint amount) public {
        ...
        IBEP20(address(dino)).safeTransfer(msg.sender, amount);
    ...
    function claim(address dino20) public {
        ...
        IBEP20(dino20).safeTransfer(msg.sender, userOfferingAmount);
        IBEP20(address(dino)).safeTransfer(dino.staker(), feeAmount);
        IBEP20(address(dino)).safeTransfer(msg.sender, userAmount.sub(feeAmount));
    ...
    function claimOwner(address dino20) public {
        ...
        require(block.number >= pool.endBlock, "Dino: not over");
        uint currentBalance = IBEP20(dino20).balanceOf(address(this));

        if(currentBalance > pool.offeringAmount) {
            uint ownerBalance = currentBalance.sub(uint(pool.offeringAmount));

            IBEP20(dino20).safeTransfer(pool.owner, ownerBalance);
